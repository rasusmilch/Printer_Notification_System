...
volatile uint32_t wdt_loop_start_time, wdt_loop_end_time;

// Arrays to store minimum and maximum times for each task
volatile uint32_t wdt_min_times[5] = {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF,
                                      0xFFFFFFFF, 0xFFFFFFFF};
volatile uint32_t wdt_max_times[5] = {0, 0, 0, 0, 0};
...
const unsigned long TASK1_MAX_TIME = 50;
const unsigned long TASK2_MIN_TIME = 0;
const unsigned long TASK2_MAX_TIME = 50;
const unsigned long TASK3_MIN_TIME = 0;
const unsigned long TASK3_MAX_TIME = 50;
const unsigned long TASK4_MIN_TIME = 0;
const unsigned long TASK4_MAX_TIME = 50;

// Define pin numbers
#define PIN_BTN_LIGHT 7
#define PIN_BTN_BUZZER 8
#define PIN_BTN_LIGHT_LED_RED 10
#define PIN_BTN_LIGHT_LED_GRN 3
#define PIN_BTN_BUZZER_LED_RED 11
#define PIN_BTN_BUZZER_LED_GRN 9
#define PIN_POWER_LED 5
#define PIN_NOTIFICATION_LIGHT 4
#define PIN_NOTIFICATION_BUZZER 6
#define PIN_PRINTOUT 12

// Define timing constants
#define STROBE_SLOW_OFF_MS 20000UL
#define STROBE_SLOW_ON_MS 200UL
#define STROBE_FAST_OFF_MS 2000UL
#define STROBE_FAST_ON_MS 200UL
#define BUZZER_ONCE_ON_MS 1000UL

// Define debounce delay and printout old time constants
#define DEBOUNCE_DELAY 50UL
#define PRINTOUT_OLD_MS 60UL*10UL*1000UL // When is a printout considered old, so increase urgency (ms)
...
bool check_printout() {
  ...
  //Serial.print(F("Printout:"));
  //Serial.print(reading);

  // Check if the button state has changed
  if (reading != last_state) {
    lastDebounceTime = current_time; // Reset the debouncing timer
  }
  ...
  if ((current_time - lastDebounceTime) > DEBOUNCE_DELAY) {
    ...
    if (reading != printout_state) {
      ...
      if (printout_state == HIGH) {
        ...
        // Something in tray
        unsigned long pressDuration = current_time - lastDebounceTime;
...
void setup() {
  ...
  pinMode(PIN_PRINTOUT, INPUT_PULLUP);
...
void wdt_loop_end() {
  ...
  wdt_loop_end_time = millis();

  difference = wdt_loop_end_time - wdt_loop_start_time;

  if (difference < wdt_min_times[4]) {
    wdt_min_times[4] = difference;
  }

  if (difference > wdt_max_times[4]) {
    wdt_max_times[4] = difference;
  }
  interrupts();                  // Re-enable interrupts
}
...
void printTaskTimings() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    Serial.print("], ");          // Print units and end the line
  }

    Serial.print(", Loop: [");
    Serial.print(wdt_min_times[4]);
    Serial.print(",");
    Serial.print(wdt_max_times[4]);
    Serial.println("]");
}
